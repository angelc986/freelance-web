    ...
    <div className="space-y-1.5">
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          <input
            ...
            className="w-full px-3 py-2 text-[14px] bg-[#F1F5F9] border border-[#E2E8F0] rounded-lg text-[#1E293B] font-medium outline-none transition-all placeholder:text-[#94A3B8] focus:border-primary focus:bg-white focus:shadow-[0_0_0_3px_rgba(37,99,235,0.12)]"
          />
            ...
            <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
              {results.map((item, i) => (
                <button key={i} type="button" onMouseDown={() => selectResult(item)}
                  className="w-full text-left px-3 py-2 text-[12px] text-gray-700 hover:bg-blue-50 hover:text-primary border-b border-gray-50 last:border-0 transition-colors"
                >
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex-1 py-1.5 text-[12px] font-medium text-primary bg-blue-50 hover:bg-blue-100 rounded-lg border border-blue-100 transition-all flex items-center justify-center gap-1"
        >
              ...
              <div className="relative">
                ...
                {showResults && results.length > 0 && (
                  <div className="absolute top-full left-0 right-0 z-[1100] mt-1.5 bg-white border border-gray-200 rounded-xl shadow-lg shadow-black/5 max-h-40 overflow-y-auto modal-scroll">
                    {results.map((item, i) => (
                      <button key={i} type="button" onMouseDown={() => selectResult(item)}
                        className="w-full text-left px-3.5 py-2.5 text-[13px] text-gray-700 hover:bg-primary-lighter hover:text-primary border-b border-gray-50 last:border-0 transition-colors flex items-start gap-2"
                      >
