...
cats = ['Gastronomia','Logistica','Servicios','Limpieza','Eventos','Retail','Construccion','Mudanza','Oficina','Delivery']
locs = ['Caracas','Maracay','Valencia','Barquisimeto','Maracaibo','Tacoma','Miami','Bogota','Lima','Santiago']
...
for i in range(15):
    ...
    r = requests.post(BASE + '/jobs', headers=auth_hdr(ctok),
        json={'title': title, 'description': 'Trabajo demo #' + str(i+1),
              'category': cat, 'location': loc, 'budget': budget, 'duration': '3h'}, timeout=10)
    ...
    r = requests.post(BASE + '/jobs/' + str(jid) + '/apply', headers=auth_hdr(wtok),
    ...
    r = requests.get(BASE + '/jobs/' + str(jid) + '/applications', headers=auth_hdr(ctok), timeout=10)
    apps = r.json()
    ...
    r = requests.post(BASE + '/jobs/' + str(jid) + '/accept/' + str(app_id), headers=auth_hdr(ctok), json={}, timeout=10)
    if r.status_code != 200:
        print('  ' + str(i+1) + '/15: FAIL accept HTTP ' + str(r.status_code))
        continue
    time.sleep(0.1)

    r = requests.post(BASE + '/jobs/' + str(jid) + '/check-in', headers=auth_hdr(wtok), json={}, timeout=10)
    if r.status_code != 200:
        print('  ' + str(i+1) + '/15: FAIL checkin HTTP ' + str(r.status_code))
        continue
    time.sleep(0.1)

    r = requests.post(BASE + '/jobs/' + str(jid) + '/complete-request', headers=auth_hdr(wtok), json={}, timeout=10)
    if r.status_code != 200:
        print('  ' + str(i+1) + '/15: FAIL complete HTTP ' + str(r.status_code))
        continue
    time.sleep(0.1)

    r = requests.post(BASE + '/jobs/' + str(jid) + '/approve', headers=auth_hdr(ctok), json={}, timeout=10)
    if r.status_code != 200:
    ...
    if r.status_code != 200:
        print('  ' + str(i+1) + '/15: FAIL release HTTP ' + str(r.status_code) + ' - ' + r.text[:80])
        continue
...
for log in db.query(AuditLog).order_by(AuditLog.id.desc()).limit(10).all():
    print('  #' + str(log.id) + ' user=' + str(log.user_id) + ' action=' + log.action + ' ip=' + str(log.ip_address or '-'))
db.close()
