...
r = requests.post(BASE + '/payments/deposit', headers=hdr(ctok),
    json={'amount': 1000, 'tx_hash': '0xabc123demo' + str(int(time.time()))}, timeout=10)
...
for i in range(5):
    ...
    r = requests.post(BASE + '/jobs', headers=hdr(ctok),
        json={'title':'Demo ' + str(11+i) + ' - ' + cat, 'description':'Trabajo demo #' + str(11+i),
    ...
    r = requests.post(BASE + '/jobs/'+str(jid)+'/apply', headers=hdr(wtok), json={'message':'test'}, timeout=10)
    if r.status_code not in (200,201):
    ...
    r = requests.post(BASE + '/jobs/'+str(jid)+'/accept/'+str(app_id), headers=hdr(ctok), json={}, timeout=10)
    if r.status_code != 200:
        continue
    time.sleep(0.08)

    r = requests.post(BASE + '/jobs/'+str(jid)+'/check-in', headers=hdr(wtok), json={}, timeout=10)
    if r.status_code != 200:
        continue
    time.sleep(0.08)

    r = requests.post(BASE + '/jobs/'+str(jid)+'/complete-request', headers=hdr(wtok), json={}, timeout=10)
    if r.status_code != 200:
        continue
    time.sleep(0.08)

    r = requests.post(BASE + '/jobs/'+str(jid)+'/approve', headers=hdr(ctok), json={}, timeout=10)
    if r.status_code != 200:
        continue
    time.sleep(0.08)
