...
r = requests.post(BASE + '/jobs',
    ...
    json={'title': 'Prueba Flujo ' + ts, 'description': 'Test del flujo completo',
...
# 1. Aplicar
def step1():
    r = requests.post(BASE + '/jobs/' + str(JOB_ID) + '/apply',
...
def step2():
    global APP_ID
    r = requests.get(BASE + '/jobs/' + str(JOB_ID) + '/applications',
...
# 3. Aceptar
def step3():
    r = requests.post(BASE + '/jobs/' + str(JOB_ID) + '/accept/' + str(APP_ID),
        headers={'Authorization': 'Bearer ' + CTOK, 'Content-Type': 'application/json'},
        json={}, timeout=10)
...
# 4. Check-in
def step4():
    r = requests.post(BASE + '/jobs/' + str(JOB_ID) + '/check-in',
        headers={'Authorization': 'Bearer ' + WTOK, 'Content-Type': 'application/json'},
        json={}, timeout=10)
    assert r.status_code == 200, 'HTTP ' + str(r.status_code) + ': ' + r.text[:200]
...
# 5. Complete-request
def step5():
    r = requests.post(BASE + '/jobs/' + str(JOB_ID) + '/complete-request',
        headers={'Authorization': 'Bearer ' + WTOK, 'Content-Type': 'application/json'},
        json={}, timeout=10)
    assert r.status_code == 200, 'HTTP ' + str(r.status_code) + ': ' + r.text[:200]
...
# 6. Approve
def step6():
    r = requests.post(BASE + '/jobs/' + str(JOB_ID) + '/approve',
        headers={'Authorization': 'Bearer ' + CTOK, 'Content-Type': 'application/json'},
        json={}, timeout=10)
    assert r.status_code == 200, 'HTTP ' + str(r.status_code) + ': ' + r.text[:200]
...
# 7. Release
def step7():
    r = requests.post(BASE + '/payments/release/' + str(JOB_ID),
        headers={'Authorization': 'Bearer ' + CTOK}, timeout=10)
    assert r.status_code == 200, 'HTTP ' + str(r.status_code) + ': ' + r.text[:200]
    d = r.json()
    st = d.get('status')
