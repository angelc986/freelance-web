...
contractor_token, contractor_user = login(
    {"email": "test_contractor@test.com", "password": "test123"}
)
...
for i in range(30):
    ...
    if code not in (200, 201):
        check("1. Crear trabajo", False, f"HTTP {code}")
        continue
    job_id = data.get("id")
    ...
    check("2. Aplicar", code in (200, 201), f"HTTP {code}")
    time.sleep(0.2)
...
for log in db.query(AuditLog).order_by(AuditLog.id.desc()).limit(15).all():
    print(
        f"    [{log.id:3d}] user={log.user_id:2d} {log.action:25s} ip={log.ip_address or '-'}",
        flush=True,
    )
db.close()
