...
if not r.ok:
    ...
    r = requests.post(
        BASE + "/auth/register",
        json={
            "email": "rate_worker@test.com",
            "phone": "+501",
            "full_name": "Rate Worker",
            "cedula": "55555",
            "password": PASS,
            "role": "worker",
        },
    ...
    r = requests.post(
        BASE + "/auth/login", json={"email": "rate_worker@test.com", "password": PASS}
    )
...
if not r.ok:
    r = requests.post(
        BASE + "/auth/register",
        json={
            "email": "rate_contractor@test.com",
            "phone": "+502",
            "full_name": "Rate Contractor",
            "cedula": "66666",
            "password": PASS,
            "role": "contractor",
        },
    ...
    r = requests.post(
        BASE + "/auth/login", json={"email": "rate_contractor@test.com", "password": PASS}
    )
...
if not completed:
    ...
    r = requests.post(
        BASE + "/jobs/",
        json={
            "title": "Test rating job",
            "description": "Testing rating feature. Need worker for quick test.",
            "category": "Servicios",
            "location": "Test",
            "budget": 10,
            "duration": "1-2 horas",
    ...
    # Get application
    r = requests.get(
        BASE + "/jobs/" + str(job_id) + "/applications", headers={"Authorization": "Bearer " + tk_c}
    )
    app_id = r.json()[0]["id"]
    ...
    # Check-in
    r = requests.post(
        BASE + "/jobs/" + str(job_id) + "/check-in", headers={"Authorization": "Bearer " + tk_w}
    )
    ...
    # Approve
    r = requests.post(
        BASE + "/jobs/" + str(job_id) + "/approve", headers={"Authorization": "Bearer " + tk_c}
    )
    print("Job completed!")
else:
