...
# 1. Login contractor
r = requests.post(f'{BASE}/auth/login', json={'email':'angelcurbelo901@gmail.com','password':'test123'}, timeout=10)
ctok = r.json()['access_token']
user = r.json()['user']
print(f'[OK] 1. Login contractor: {user["full_name"]} (ID={user["id"]})')

# 2. Login worker
r = requests.post(f'{BASE}/auth/login', json={'email':'instaworkve@gmail.com','password':'test123'}, timeout=10)
...
r = requests.post(f'{BASE}/jobs',
    ...
    json={'title': 'Flujo Completo Test', 'description': 'Prueba unica del flujo completo',
          'category': 'Servicios', 'location': 'Caracas', 'budget': 50, 'duration': '2h'},
    timeout=10)
job_id = r.json()['id']
...
# 5. Apply
r = requests.post(f'{BASE}/jobs/{job_id}/apply',
...
# 6. View applicants
r = requests.get(f'{BASE}/jobs/{job_id}/applications',
...
# 7. Accept
r = requests.post(f'{BASE}/jobs/{job_id}/accept/{app_id}',
    headers={'Authorization': f'Bearer {ctok}', 'Content-Type': 'application/json'},
    json={}, timeout=10)
...
# 8. Check-in
r = requests.post(f'{BASE}/jobs/{job_id}/check-in',
    headers={'Authorization': f'Bearer {wtok}', 'Content-Type': 'application/json'},
    json={}, timeout=10)
...
# 9. Complete request
r = requests.post(f'{BASE}/jobs/{job_id}/complete-request',
    headers={'Authorization': f'Bearer {wtok}', 'Content-Type': 'application/json'},
    json={}, timeout=10)
...
# 10. Approve
r = requests.post(f'{BASE}/jobs/{job_id}/approve',
    headers={'Authorization': f'Bearer {ctok}', 'Content-Type': 'application/json'},
    json={}, timeout=10)
...
# 11. Release payment
r = requests.post(f'{BASE}/payments/release/{job_id}',
    headers={'Authorization': f'Bearer {ctok}'}, timeout=10)
print(f'[OK] 11. Release pago -> HTTP {r.status_code}')
if r.status_code == 200:
    tx_data = r.json()
    print(f'     Transaccion tipo={tx_data.get("type")}, monto={tx_data.get("amount")}, status={tx_data.get("status")}')
