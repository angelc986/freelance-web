...
# Check both accounts
cur.execute("SELECT id, email, full_name, cedula, cedula_locked FROM users WHERE email IN ('Angel@gmail.com', 'angelcurbelos901@gmail.com')")
...
cur.execute("UPDATE users SET cedula = '28659265', cedula_locked = TRUE WHERE email = 'angelcurbelos901@gmail.com'")
print(f"Updated angelcurbelos901@gmail.com: {cur.rowcount} row(s)")

# Verify
cur.execute("SELECT id, email, full_name, cedula, cedula_locked FROM users WHERE email = 'angelcurbelos901@gmail.com'")
for r in cur.fetchall():
