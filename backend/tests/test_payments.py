...
def create_completed_job(client, contractor_token, worker_token):
    ...
    job_resp = client.post("/api/v1/jobs", json={
        "title": "Payable Job", "description": "test",
        "category": "Servicios", "location": "Caracas",
        "budget": 50, "duration": "2h",
    ...
    client.post(f"/api/v1/jobs/{jid}/complete-request", json={},
                headers={"Authorization": f"Bearer {worker_token}"})
    client.post(f"/api/v1/jobs/{jid}/approve", json={},
                headers={"Authorization": f"Bearer {contractor_token}"})
...
class TestRelease:
    def test_release_completed_job(self, client: TestClient, contractor_token, worker_token, db):
        ...
        resp = client.post(f"/api/v1/payments/release/{jid}",
                          headers={"Authorization": f"Bearer {contractor_token}"})
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_release_nonexistent_job(self, client: TestClient, contractor_token):
        ...
        resp = client.post("/api/v1/payments/release/99999",
                          headers={"Authorization": f"Bearer {contractor_token}"})
