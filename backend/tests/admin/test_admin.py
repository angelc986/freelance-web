...
class TestAdminAccessControl:
    ...
    def test_contractor_blocked_from_stats(self, client, contractor_token):
        ...
        resp = client.get(
            "/api/v1/admin/stats", headers={"Authorization": f"Bearer {contractor_token}"}
        )
    ...
    def test_worker_blocked_from_stats(self, client, worker_token):
        ...
        resp = client.get(
            "/api/v1/admin/stats", headers={"Authorization": f"Bearer {worker_token}"}
        )
        assert resp.status_code == 403
    ...
    def test_contractor_blocked_from_users(self, client, contractor_token):
        ...
        resp = client.get(
            "/api/v1/admin/users", headers={"Authorization": f"Bearer {contractor_token}"}
        )
        assert resp.status_code == 403
...
class TestAdminDisputes:
    ...
    def test_list_disputes(self, client, admin_token):
        ...
        resp = client.get(
            "/api/v1/admin/disputes", headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert resp.status_code == 200
        data = resp.json()
        assert isinstance(data, dict)
...
class TestAdminWallet:
    ...
    def test_wallet_info(self, client, admin_token):
        ...
        resp = client.get(
            "/api/v1/admin/wallet", headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert resp.status_code == 200
        data = resp.json()
...
class TestAdminUserDetail:
    ...
    def test_user_detail(self, client, admin_token, contractor_token):
        # Get contractor id
        me = client.get("/api/v1/auth/me", headers={"Authorization": f"Bearer {contractor_token}"})
        uid = me.json()["id"]

        resp = client.get(
            f"/api/v1/admin/users/{uid}", headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert resp.status_code == 200
    ...
    def test_user_detail_requires_admin(self, client, contractor_token):
        ...
        resp = client.get(
            f"/api/v1/admin/users/{uid}", headers={"Authorization": f"Bearer {contractor_token}"}
        )
        assert resp.status_code == 403
...
class TestAdminTransactions:
    ...
    def test_transactions_contractor_blocked(self, client, contractor_token):
        resp = client.get(
            "/api/v1/admin/transactions", headers={"Authorization": f"Bearer {contractor_token}"}
        )
        assert resp.status_code == 403
...
class TestAdminUserStatus:
    ...
    def test_toggle_status_admin(self, client, admin_token, contractor_token):
        ...
        resp = client.patch(
            f"/api/v1/admin/users/{uid}/status?is_active=false",
